...
yum install -y mysql-community-server-${mysql_version} --nogpgcheck
yum install -y mysql-shell-${mysql_version} --nogpgcheck
yum install -y mysql-router-community-${mysql_version} --nogpgcheck
rm -rf /var/lib/mysql/*
